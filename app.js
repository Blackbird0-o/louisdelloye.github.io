...
darkMode.addEventListener('click', animation);


// Automation of dark mode
// Calculate the sun's positions
/*
  Adapted from :
...
function showPosition(position) {
  ...
  // Get sun times
  var suntimes = window.SunCalc.getTimes(new Date(), lat, long);

  var sunrise = suntimes.sunrise;
  var sunset = suntimes.sunset;

  // Determine if it is day or night
  if (!(sunrise.getTime() < Date.now() && sunset.getTime() > Date.now())) {
    // Animate to dark mode if it is night time where you are
    animation();
  }
...
// Update dark/light mode based on if it is night or day
updateAppearance();